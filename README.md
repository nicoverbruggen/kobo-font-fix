...
4. **PANOSE metadata is checked and fixed.** Sometimes, the PANOSE information does not match the font style. This is often an oversight but it causes issues on Kobo devices, so this fixes that. An existing weight is also corrected when the PANOSE family is unspecified; other PANOSE fields stay unchanged in that case.
